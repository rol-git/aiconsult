...
AGENT_HINTS = {
    ...
    AgentType.GEO: "ближайший ПВР, проверка зоны подтопления, перекрытия дорог, гидропосты, расстояние, маршруты в 2ГИС/Яндекс.Картах",
}
...
AGENT_FOLLOWUPS = {
    ...
    AgentType.GEO: [
        "Где находится ближайший пункт временного размещения?",
        "Моё здание попадает в зону подтопления?",
        "Какие дороги перекрыты рядом со мной?",
        "Покажи маршрут от меня до ближайшего ПВР.",
    ],
}
